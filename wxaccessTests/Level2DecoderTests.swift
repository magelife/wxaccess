...
struct Level2DecoderTests {
    ...
    func bzip2RoundTrip() throws {
        let compressed = Data([
            0x42, 0x5a, 0x68, 0x39, 0x31, 0x41, 0x59, 0x26, 0x53, 0x59, 0x40, 0xcb,
            0x43, 0xa5, 0x00, 0x00, 0x02, 0x17, 0x00, 0x40, 0x00, 0x26, 0x41, 0x10,
            0x40, 0x02, 0x04, 0xa0, 0x00, 0x31, 0x00, 0xd3, 0x4d, 0x04, 0x03, 0x4c,
            0x87, 0x1a, 0x20, 0x22, 0xb7, 0x9b, 0xc5, 0xdc, 0x91, 0x4e, 0x14, 0x24,
            0x10, 0x32, 0xd0, 0xe9, 0x40
        ])
        #expect(try bzip2Decompress(compressed) == Data("Hello NEXRAD".utf8))

        let withTrailingData = compressed + Data([0x00, 0x01, 0x02])
        #expect(try bzip2Decompress(withTrailingData) == Data("Hello NEXRAD".utf8))
